...
from mudipu.config import MudipuConfig, get_config
...
@click.option(
    "--output", "-o", type=click.Path(path_type=Path), help="Save summary to file instead of printing to console."
)
...
@click.option(
    "--visualize", "-v", is_flag=True, help="Generate matplotlib visualization chart with health trends and breakdowns."
)
@click.option(
    "--output-dir",
    "-o",
...
def health(
    ...
    try:
        ...
        with Progress(
            ...
            try:
                ...
                metrics = analyzer.get_health_metrics(
                    goal=goal, include_viz=visualize, output_dir=output_dir, progress_callback=update_progress
                )
        ...
        # Display header
        console.print(Panel(f"[bold]{session.name or 'Trace Session'}[/bold]\n" f"Health Analysis", style="cyan"))
        ...
        summary_table.add_row(
            "[bold]Overall Health[/bold]", f"[bold]{overall_score}[/bold]", f"[bold]{overall_interp}[/bold]"
        )
        ...
        summary_table.add_row(
            "Context Growth Rate", f"[{growth_color}]{growth_rate:.1f}[/{growth_color}]", "tokens/turn"
        )
