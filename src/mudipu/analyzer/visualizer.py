...
class HealthVisualizer:
    ...
    def create_health_report(
        self, session: Session, analysis: HealthAnalysis, output_dir: Optional[Path] = None
    ) -> Path:
        """
        ...
        try:
            ...
            raise ImportError("matplotlib is required for visualizations. " "Install with: pip install mudipu[health]")
        ...
        fig.suptitle(f"Health Report: {session.name or 'Session ' + session_id_short}", fontsize=16, fontweight="bold")
    ...
    def _plot_context_growth(self, ax, session: Session):
        ...
        tokens = [t.usage.get("prompt_tokens", 0) if t.usage else 0 for t in session.turns]
        ...
        for i, turn in enumerate(session.turns):
            if turn.tool_calls_detected:
                tool_names = [tc.function_name for tc in turn.tool_calls_detected if tc.function_name]
                if tool_names:
    ...
    def plot_token_growth_simple(self, session: Session, output_path: Optional[Path] = None) -> Optional[Path]:
        """
        ...
        try:
            ...
            raise ImportError("matplotlib is required. Install with: pip install mudipu[health]")
