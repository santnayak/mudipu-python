...
def _get_sentence_transformer():
    ...
    if _sentence_transformer is None:
        try:
            ...
            raise ImportError(
                "sentence-transformers is required for health metrics. " "Install with: pip install mudipu[health]"
            )
...
class HealthAnalysis:
    ...
    def to_dict(self) -> dict:
        """Convert to dictionary."""
        return {"per_turn": [t.to_dict() for t in self.per_turn], "session": self.session.to_dict()}
...
class ContextHealthAnalyzer:
    ...
    def get_full_analysis(self) -> HealthAnalysis:
        ...
        return HealthAnalysis(per_turn=turn_metrics, session=session_metrics)
